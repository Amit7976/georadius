...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedCategory === "All") {
      ...
      setFilteredNews(
        newsData.filter((news) => news.categories.includes(selectedCategory))
      );
    }
  ...
  return (
    ...
    <div className="h-screen w-full bg-white p-0 scroll-smooth">
      {/* Normal Header (Shadow only when scrolled) */}
      ...
      {/* Fixed Header (Shows when scrolling UP) */}
      <div
        className={clsx(
          "fixed top-0 left-0 w-full bg-white transition-transform duration-300 z-50",
          showFixedHeader ? "translate-y-0" : "-translate-y-full"
        )}
        style={{ boxShadow: addShadow ? "0 4px 6px rgba(0, 0, 0, 0.1)" : "none" }}
      >
        ...
        {/* Fixed Categories (Only when original categories are out of view) */}
        {showFixedCategories && (
          <div className="w-full py-2 pt-0 bg-white">
            <div className="flex gap-2 px-2 overflow-x-auto whitespace-nowrap">
              {categoryOptions.map((category) => (
                <Button
                  key={category}
